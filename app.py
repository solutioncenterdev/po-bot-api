...
def index():
    ...
    reply,index,instanceID,created_by_user,SupplierName,PurchaseOrderNetAmount,after_approval_reply,all_item_details,no_of_line_items = query_get_task_with_details(data['conversation']['memory'],present_skill,bot_nlp)
    return jsonify(
        status=200,
        replies=[{
        'type': 'text',
...
def query_get_task_with_details(bot_memo,present_skill,bot_nlp):
    ...
    if ((bot_memo == {} or bot_memo['index']) and present_skill == 'get_task'):
        ...
        if ((len(body1["d"]["results"])==1)):
            ...
            return final_reply_string,bot_memo['index'] + 1,instance_id,created_by_user,SupplierName, (PurchaseOrderNetAmount + ' ' + DocumentCurrency),'',all_item_details,no_of_line_items
    ...
    #         return final_reply_string,bot_memo['index'],len(body1["d"]["results"]),created_by_user,SupplierName, (PurchaseOrderNetAmount + ' ' + DocumentCurrency)
        ...
        return  item_level_reply_ordinally,bot_memo['index'],bot_memo['instanceID'],bot_memo['created_by_user'], bot_memo['SupplierName'],bot_memo['PurchaseOrderNetAmount'],bot_memo['after_approval_reply'],bot_memo['all_item_details'],bot_memo['no_of_line_items']
